    ...
    while kb < n {
        ...
        for jb in 0..p_full / NR {
            ...
            for ib in 0..m_full / MR {
                let i0 = ib * MR;
                // SAFETY: `i0 + 32 <= m_full <= m` and `j0 + 4 <= p_full <= p` by the
        ...
        while i0 + 16 <= m {
            for jb in 0..p_full / NR {
                let j0 = jb * NR;
                // SAFETY: the loop condition guarantees `i0 + 16 <= m`, the `jb` loop
        ...
        for j in p_full..p {
            for k in kb..k_end {
                ...
                // stores one 16-lane vector at `offset = i·16` with `i < i_simd = m / 16`,
                // so `offset + 16 <= m`: every access stays inside column `k` of `a`
...
///
/// - `i0 + 32 <= m`, so the tile's 32 rows are within `a`'s and `c`'s columns;
