...
- **Filtering & convolution**: `convolve2d` (any `MatrixRef` kernel), `convolve2d_separable`, `gaussian_blur`, `box_blur`, allocation-free `_into` variants (`convolve2d_separable_into`, `gaussian_blur_into` — reuse caller-owned buffers) and streaming `_cols` variants (`convolve2d_separable_cols`, `gaussian_blur_cols` — banded, per-output-column callback, never materializes the result; bit-identical to the allocating calls), `gaussian_blur_kernel`, `unsharp_mask`, `laplacian`, `laplacian_of_gaussian`, `sobel_gradients`, `scharr_gradients`, `gradient_magnitude`
